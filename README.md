...
건축 실무에서 AI 코딩 에이전트와 함께 일하며 굳어진 작업 절차를 스킬로 정리한 저장소입니다. 각 스킬은 `skills/<이름>/SKILL.md` 하나로 시작하고, 필요할 때만 참고 문서와 스크립트를 덧붙입니다. 실제 실행 기록과 판정은 `evidence/<이름>/`에 따로 두어, 스킬 폴더만 설치하면 문서와 스크립트(수백 KB)만 들어옵니다. Codex와 Claude Code 양쪽에서 같은 파일을 씁니다.

Reusable agent skills distilled from day-to-day architectural practice with AI coding agents. Each skill lives in `skills/<name>/SKILL.md` and works in both Codex and Claude Code. Real-run evidence and verdicts live separately in `evidence/<name>/` so that installing a skill stays light.
